...
class SearchAndExtract100:
    ...
    def submit_search_and_wait(self, driver):
        ...
        try:
            ...
            # Click the search button like a real user (not form.submit() which is bot-detectable)
            try:
    ...
    def save_results(self):
        ...
        try:
            # Save JSON report
            json_file = os.path.join(PDKI_DIR, f"search_100_results_{timestamp}.json")
...
def main():
    ...
    try:
        ...
        timestamp = int(time.time())
        final_screenshot = os.path.join(PDKI_DIR, f"final_results_{timestamp}.png")
